...
ValueType Deserialize(const gsl::span<const gsl::byte> span) {
  ...
  ASSERT_PATTER(span.size() == N * sizeof(uint64_t), "Source span size mismatches BigInt size.");
...
void Serialize(const ValueType& val, const gsl::span<gsl::byte> span_out) {
  const size_t N = ValueType::LimbCount();
  ASSERT_PATTER(span_out.size() == N * sizeof(uint64_t), "Span size mismatches BigInt size.");
